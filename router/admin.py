...
# 获取待审核房源列表
@router.get("/audit/list", summary="获取待审核房源列表", status_code=status.HTTP_200_OK)
async def get_pending_houses_api(
        current_user: User = Depends(utils.auth.get_current_user),
        db: AsyncSession = Depends(get_database)
):
    """获取待审核房源列表"""
    houses = await get_pending_houses(db)
    res_data = [HouseResponse().model_validate(house) for house in houses]
    return success_response(message="获取成功", data=res_data)
...
async def verify_house_api(
    ...
    await verify_house(db, house_id, status)
    return success_response(message="审核成功")


# 获取用户列表
@router.get("/users", summary="获取用户列表", status_code=status.HTTP_200_OK)
async def get_users_api(
    ...
    res_data = [SafeUserResponse().model_validate(user) for user in users]
    return success_response(message="获取成功", data=res_data)
